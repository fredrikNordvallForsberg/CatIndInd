...
\usepackage{etex} %needed because todonotes loads tikz
...
\cite{ghaniJohannFumex2010fibind} %todo: reword citation -- what's new with Ghani et al?
...
%initial [fancy name for objects in $\mathbb{E}$]
...
$\UFamSet \circ \ArgB = \ArgA \circ \UDialgArgAU$. %todo: last explanation bit weak?
...
\end{lemma}
%\begin{proof}
%  We can define $\ArgB(A, B, c) = (\ArgA(A, B), \ArgB''(A, B, c))$ in both directions. \qed
%\end{proof}
...
A \to \Set$ and morphisms which can be split up into %todo: fix
\begin{align*}
...
\begin{align*}
  \ArgCtxt(A, B) &= \one + \Sigma\, \Gamma\!:\!A .\, B(\Gamma)\\
  \ArgType''(A, B, c, x) &= \one + (\Sigma\,  \sigma\!:\! B(c(x)).\, \tau\!:\!B(c(\inr(c(x), \sigma))) ) \enspace .
\end{align*} %todo: check development
...
\subsection{Functors from the earlier axiomatisation} %TODO: Better title
...
next section) hold %todo: better word?
...
\end{align*}
%todo: think of notation; \overline{c} instead of f?

%todo: yay, this part needs rewriting! [even more than other parts...]
...
%
  \BOX_{\ArgB}(P, Q) :\ %&(c : \ArgA(A, B) \to A) \to \\
...
by $\sigmamorph{(f, g)}{(h, k)} = \CwFmorph{(f, g) \circ (\pi_0, \pi'_0)}{(h, k)}$
 and $\CwFsect{f} = \CwFmorph{\id}{f}$. %todo: still a mess
...
% \begin{remark}
%   If the definition of the eliminator for an inductive-inductive
%   definition seems complicated, one might take comfort in knowing that
%   it actually is an instance of a general eliminator for functors between
%   categories with families (introduced by Dybjer
%   \cite{dybjer1996internalTT}). Unfortunately, we have to refrain from
%   investigating this line of research in this paper due to space
%   constraints.
% \end{remark}
...
\begin{proof}
  ...
  $\Eind{\ArgA}{\ArgB}$.
  % since the first component of $\sigmamorph{(c, d)}{(f, g)} \circ
  % (\varphi_{\ArgA}, \varphi_{\ArgB})$ is $\sigmamorph{c}{f} \circ
  % \varphi_{\ArgA}$, which coincides with the ``constructor'' in
  % $\SigmaDialg (A, B, c)\ (P, Q, f)$.
...
\begin{proof}
  ...
  Since
   ...
   \begin{align*} %todo: same notation as \CwFsect
...
% For every pair $(\ArgA, \ArgB'')$ of functors arising from a code
...
% \end{align*}
% \begin{align*}
%   \ArgB''(A, B, c, y) &\cong \{ (s : S') \times (k : K'(s) \to A)
...
%   \ArgB''(f, g)(\langle s, k, l_0, &\ldots, \l_n\rangle) = \langle s, f \circ k, \lambda x y .\, g(k(x), l_0(x, y)), \\
...
% For functors $(ArgA, \ArgB'')$ as in Lemma \ref{thm:CSL-functor-CNF},
% we have
% \begin{multline*}
