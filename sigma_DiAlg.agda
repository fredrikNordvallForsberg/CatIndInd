...
,-cong : {A : Set}{B : A -> Set} -> {x x' : A}{y : B x}{y' : B x'} ->
         (p : x == x') -> subst B p y == y' -> _==_ {Σ (A , B)} (x , y) (x' , y')
,-cong refl refl =  refl
...
boxF A B P Q x =  Σ (F (ΣFAM A B P Q) , λ y → Fmor (record { f = π₀; g = λ x z → π₀ z }) y == x )
...
boxG A B P Q inF inFbar x b y = Σ (G (ΣFAM A B P Q , (ΣMor (inF , inFbar) ∘ φ {Q = Q})) (π₀ y) , λ z → subst (G ((A , B) , inF)) (π₁ y) (Gmor ((π₀ , λ x → π₀) , refl) (π₀ y) z) == b)
...
dmapG A B P Q inF inFbar f g x y = (Gmor ((((λ x → x , f x)) , λ x y → y , g x y) , λ {x} → ,-cong (cong inF (symm (trans Ffunctorlawcomp Ffunctorlawid))) {!!}) x y) , primTrustMe
